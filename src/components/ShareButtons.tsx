...
export function ShareButtons({ baslik, slug }: Props) {
    ...
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`${baslik}\n${url}`)}`;
    ...
    const copyLink = async () => {
        try {
            ...
            /* eski tarayıcılar için sessizce geç */
        }
    };

    return (
        <div className="flex items-center gap-2">
            <span className="text-xs text-(--muted) mr-1">Paylaş:</span>
            ...
            <a
                ...
                className="w-8 h-8 rounded-full border border-[var(--border)] flex items-center justify-center
                   hover:border-[var(--accent)] hover:text-[var(--accent)] transition-all text-sm"
            >
                𝕏
            </a>
            ...
            <a
                ...
                className="w-8 h-8 rounded-full border border-[var(--border)] flex items-center justify-center
                   hover:border-green-500 hover:text-green-500 transition-all text-sm"
            >
